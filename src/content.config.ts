...
// 2026-09-17 課程分類——同一套 years/semesters 的 relation collection 模式。
// 觀察:82 筆 115 春季課程的代碼中間那個英文字母(a/b/c)其實已經對應到學校選課
// 系統原本的課程分類(從課程標題比對出的規律:a≈才藝人文類、c≈健康生活類、
// b≈社團行動類,只有 2 筆樣本,不確定是否為官方正式分類名稱)。與其寫死這三個
// 選項,一樣開放成 CMS 可管理清單,同仁可以之後自己改名/增加分類,程式碼不用
// 再改。已用這個字母對照規則幫既有 82 筆資料回填預設分類(見資料遷移紀錄),
// 同仁看到不準的地方歡迎直接在後台改掉,不影響網站運作。
